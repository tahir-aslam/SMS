...
namespace SMS.DAL
{
    public class AdmissionDAL
    {
        ...
        public List<admission> get_all_admissions()
        {
            ...
            using (MySqlConnection con = new MySqlConnection(Connection_String.m_Connection_String.ConnectionString))
        ...
        public List<admission> getAdmissionsBySectionID(int section_id)
        {
            ...
            using (MySqlCommand cmd = new MySqlCommand())
            {
                cmd.CommandText = "SELECT* FROM sms_admission where is_active='Y' && section_id=@section_id && session_id=" + MainWindow.session.id;
                cmd.Connection = con;
